...
var pollServers = async () => {
   anyActiveServer = false;
   //needs to be a plain for loop because a forEach does not play nicely with await used below
   for (var serverIndex = 0; serverIndex < knownservers.length; serverIndex++) {
       let server = knownservers[serverIndex];
       let address = server[1].split(':')
       let persistedState = server[3];
       let state = null;

       try {
           //using await here so we can synchronously figure out if there are any active servers to determine our next sleep
           //you could easily asynchronously query all the servers if you didn't care about their aggregated state
           state = await Gamedig.query({'type': 'quake3', 'host': address[0], 'port': address[1]});
       } catch (error) {
           persistedState.lastPlayerCount = 0;
           persistedState.lastPlayerCountChangedTime = new Date();
           persistedState.lastThresholdHit = 0;
           persistedState.lastThresholdHitTime = new Date();
           persistedState.hasNotifiedEveryone = false;
       }
       ...
       if (state) {
           ...
           if (playerCount != persistedState.lastPlayerCount) {
               console.log(`${server[0]} player count changed from ${persistedState.lastPlayerCount} to ${playerCount}`)
               if (playerCount > 0 && data.activeThresholds) {
                   let greatestThreshold = null;
                   for (var i = 0; i < data.activeThresholds.length; i++) {
                       if (playerCount >= data.activeThresholds[i].threshold) {
                           greatestThreshold = data.activeThresholds[i];
                       } else {
                           break;
                       }
                   }

                   if (greatestThreshold) {
                       if (greatestThreshold.threshold > persistedState.lastThresholdHit) {
                           console.log(`${server[0]} raised threshold from ${persistedState.lastThresholdHit} to ${greatestThreshold.threshold}`)
                           let message = "{0} player{1} active, {2} on {3} [{4}]! Join in now: `/connect {5}`".format(
                               playerCount,
                               playerCount == 1 ? "" : "s",
                               greatestThreshold.description,
                               server[2],
                               server[0],
                               state.connect
                           );
                           chattychannel.send(message);
                           persistedState.lastThresholdHit = greatestThreshold.threshold;
                           persistedState.lastThresholdHitTime = new Date();
                       }
                   }
               }

               persistedState.lastPlayerCount = playerCount;
               persistedState.lastPlayerCountChangedTime = new Date();
           }
           ...
           //reset threshold after 5 minutes
           if (playerCount == 0 && persistedState.lastThresholdHit > 0 && (new Date() - persistedState.lastThresholdHitTime) > (5 * 60 * 1000)) {
               persistedState.lastThresholdHit = 0;
               persistedState.lastThresholdHitTime = new Date();
           }

           if (playerCount == 11 && (new Date() - persistedState.lastPlayerCountChangedTime) > 60000 && persistedState.hasNotifiedEveryone == false) {
               let message = "{0} [{1}] is short just 1 player for 6v6! Join in now: `/connect {2}`".format(
                   server[2],
                   server[0],
                   state.connect
               );
               chattychannel.send(message);
               persistedState.hasNotifiedEveryone = true;
           }
       }
   }

   let delay = anyActiveServer ? 5000 : 60000;
   setTimeout(pollServers, delay);
}
...
bot.on('ready', evt => {    
    ...
    if (config.chattychannel) {
        chattychannel = bot.channels.get(config.chattychannel);
        if (chattychannel) {
            console.log(`Chatty channel found: # ${chattychannel.name} "${chattychannel.topic}"`);
            knownservers.forEach(server => {
                //this is maybe not ideal, but push an active pinging state onto servers if we have a chatty channel
                server.push({
                    'lastPlayerCount': 0,
                    'lastPlayerCountChangedTime': new Date(),
                    'lastThresholdHit': 0,
                    'lastThresholdHitTime': new Date(),
                    'hasNotifiedEveryone': false
                });
            });
            setTimeout(pollServers, 1000);
        } else {
            console.log('Chatty channel not found.');
        }
    }
});
